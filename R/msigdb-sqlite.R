...
msigdb_sqlite <- function(x) {
  ...
  db_list$MSigDB <- as_tibble(tbl(db, "MSigDB"))
  db_list$MSigDB <- filter(db_list$MSigDB, .data$version_name == mdb_version)
  ...
  db_list$gene_set <- as_tibble(tbl(db, "gene_set"))
  db_list$gene_set <- select(
    db_list$gene_set,
    .data$id,
    .data$standard_name,
  ...
  db_list$gene_set_details <- as_tibble(tbl(db, "gene_set_details"))
  db_list$gene_set_details <- select(
  ...
  db_list$gene_symbol <- as_tibble(tbl(db, "gene_symbol"))
  db_list$gene_symbol <- select(
    db_list$gene_symbol,
    .data$id,
    .data$symbol,
    .data$NCBI_id
  ...
  db_list$namespace <- as_tibble(tbl(db, "namespace"))
  ...
  db_list$collection <- as_tibble(tbl(db, "collection"))
  db_list$collection <- select(
    db_list$collection,
    .data$id,
    .data$collection_name,
    .data$full_name,
